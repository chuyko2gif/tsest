...
export default function SendStep({ 
  ...
  const LoadingOverlay = () => (
    <div className={`fixed inset-0 z-[99999] ${isLight ? 'bg-gradient-to-br from-gray-100 via-white to-gray-100' : 'bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950'} flex items-center justify-center`} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, width: '100vw', height: '100vh' }}>
      {/* Мягкие фоновые круги */}
      <div className="absolute inset-0 overflow-hidden">
        <div className={`absolute top-1/4 left-1/4 w-96 h-96 ${isLight ? 'bg-violet-400/10' : 'bg-violet-500/5'} rounded-full blur-3xl animate-pulse`} style={{ animationDuration: '4s' }}></div>
        <div className={`absolute bottom-1/4 right-1/4 w-80 h-80 ${isLight ? 'bg-indigo-400/10' : 'bg-indigo-500/5'} rounded-full blur-3xl animate-pulse`} style={{ animationDuration: '5s', animationDelay: '1s' }}></div>
        <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] ${isLight ? 'bg-purple-400/5' : 'bg-purple-500/3'} rounded-full blur-3xl`}></div>
      </div>
      ...
      <div className="relative text-center max-w-md px-8">
        ...
        <div className="relative mb-10">
          <div className="w-36 h-36 mx-auto relative">
            {/* Внешнее свечение */}
            <div className={`absolute -inset-4 ${isLight ? 'bg-gradient-to-r from-violet-400/30 via-purple-400/30 to-indigo-400/30' : 'bg-gradient-to-r from-violet-500/20 via-purple-500/20 to-indigo-500/20'} rounded-full blur-xl animate-pulse`} style={{ animationDuration: '3s' }}></div>
            ...
            {/* Плавающие ноты */}
            <div className={`absolute -top-2 -right-2 ${isLight ? 'text-violet-500/70' : 'text-violet-400/60'} animate-bounce`} style={{ animationDuration: '2s' }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z"/>
              </svg>
            </div>
            <div className={`absolute -bottom-1 -left-3 ${isLight ? 'text-indigo-500/60' : 'text-indigo-400/50'} animate-bounce`} style={{ animationDuration: '2.5s', animationDelay: '0.5s' }}>
        ...
        <div className="flex justify-center gap-2">
          <span className={`w-1.5 h-1.5 ${isLight ? 'bg-violet-500/80' : 'bg-violet-400/70'} rounded-full animate-pulse`} style={{ animationDelay: '0ms' }}></span>
          <span className={`w-1.5 h-1.5 ${isLight ? 'bg-purple-500/80' : 'bg-purple-400/70'} rounded-full animate-pulse`} style={{ animationDelay: '300ms' }}></span>
          <span className={`w-1.5 h-1.5 ${isLight ? 'bg-indigo-500/80' : 'bg-indigo-400/70'} rounded-full animate-pulse`} style={{ animationDelay: '600ms' }}></span>
        </div>
      </div>
      ...
      <div className="animate-fade-up">
        <div className="mb-5 sm:mb-8">
          <div className="flex items-center gap-2.5 sm:gap-3 mb-3">
            <div className={`w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center ${isLight ? 'bg-gradient-to-br from-green-100 to-emerald-100 ring-1 ring-green-200' : 'bg-gradient-to-br from-green-500/20 to-emerald-500/20 ring-1 ring-white/10'}`}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={`sm:w-7 sm:h-7 ${isLight ? 'text-green-600' : 'text-green-300'}`}>
                <path d="M22 2L11 13"/>
                <path d="M22 2L15 22L11 13L2 9L22 2z"/>
              </svg>
            </div>
            <div>
              <h2 className={`text-xl sm:text-3xl font-black bg-gradient-to-r ${isLight ? 'from-gray-900 to-gray-600' : 'from-white to-zinc-400'} bg-clip-text text-transparent`}>Отправка на модерацию</h2>
              <p className={`text-xs sm:text-sm ${isLight ? 'text-gray-500' : 'text-zinc-500'} mt-0.5 sm:mt-1`}>Проверьте заполнение всех шагов</p>
            </div>
          </div>
        </div>
      
        {/* Статус проверки шагов */}
        <div className={`mb-4 sm:mb-6 p-4 sm:p-5 ${isLight ? 'bg-gray-50 border-gray-200' : 'bg-white/[0.02] border-white/5'} border rounded-xl`}>
          <h3 className={`font-bold text-sm sm:text-base mb-3 sm:mb-4 flex items-center gap-2 ${isLight ? 'text-gray-900' : 'text-white'}`}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="text-[#9d8df1] w-4 h-4 sm:w-5 sm:h-5">
            ...
            {requiredChecks.map((step, idx) => (
              ...
              >
                <div className="flex items-center gap-2">
                  {step.isValid ? (
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" className={`${isLight ? 'text-emerald-600' : 'text-emerald-400'} flex-shrink-0 w-4 h-4 sm:w-[18px] sm:h-[18px]`}>
                      <polyline points="20 6 9 17 4 12" strokeWidth="2"/>
                    </svg>
                  ) : (
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" className={`${isLight ? 'text-red-600' : 'text-red-400'} flex-shrink-0 w-4 h-4 sm:w-[18px] sm:h-[18px]`}>
        ...
        {!allValid && (
          <div className={`mb-4 sm:mb-6 p-3 sm:p-4 ${isLight ? 'bg-red-50 border-red-200' : 'bg-red-500/10 border-red-500/30'} border rounded-xl`}>
            <div className="flex items-start gap-2.5 sm:gap-3">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" className={`${isLight ? 'text-red-600' : 'text-red-400'} flex-shrink-0 mt-0.5 w-4 h-4 sm:w-5 sm:h-5`}>
                <circle cx="12" cy="12" r="10" strokeWidth="2"/>
                <line x1="12" y1="8" x2="12" y2="12" strokeWidth="2"/>
                <line x1="12" y1="16" x2="12.01" y2="16" strokeWidth="2"/>
              </svg>
              <div>
                <div className={`${isLight ? 'text-red-700' : 'text-red-300'} font-bold text-sm sm:text-base mb-0.5 sm:mb-1`}>Невозможно отправить релиз</div>
                <div className={`text-xs sm:text-sm ${isLight ? 'text-red-600' : 'text-red-400'}`}>
        ...
        {/* ИТОГ: Полный обзор релиза */}
        {allValid && (
          <div className={`mb-4 sm:mb-6 p-4 sm:p-5 ${isLight ? 'bg-violet-50 border-violet-200' : 'bg-gradient-to-br from-violet-500/5 to-purple-500/5 border-violet-500/20'} border rounded-xl`}>
            <h3 className={`font-bold text-sm sm:text-base mb-3 sm:mb-4 flex items-center gap-2 ${isLight ? 'text-violet-700' : 'text-violet-300'}`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">
              ...
              <div className="flex gap-3 sm:gap-4">
                {/* Обложка */}
                <div className={`w-16 h-16 sm:w-24 sm:h-24 flex-shrink-0 rounded-lg overflow-hidden ${isLight ? 'bg-gray-100 border-gray-200' : 'bg-zinc-800 border-white/10'} border`}>
                  {(coverFile || existingCoverUrl) ? (
                    ...
                    <div className={`w-full h-full flex items-center justify-center ${isLight ? 'text-gray-400' : 'text-zinc-600'}`}>
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-6 h-6 sm:w-8 sm:h-8">
                        <rect x="3" y="3" width="18" height="18" rx="2" ry="2"/>
                ...
                {/* Основная информация */}
                <div className="flex-1 min-w-0">
                  <h4 className={`font-bold ${isLight ? 'text-gray-900' : 'text-white'} text-sm sm:text-base truncate`}>{releaseTitle || 'Без названия'}</h4>
              ...
              <div className="grid grid-cols-3 gap-1.5 sm:gap-2">
                <div className={`p-2 sm:p-3 ${isLight ? 'bg-gray-50 border-gray-200' : 'bg-white/[0.03] border-white/5'} rounded-lg border`}>
                  <div className={`text-[8px] sm:text-[10px] ${isLight ? 'text-gray-500' : 'text-zinc-500'} uppercase tracking-wider`}>Треков</div>
                  <div className={`text-lg sm:text-xl font-bold ${isLight ? 'text-gray-900' : 'text-white'} mt-0.5 sm:mt-1`}>{tracksCount}</div>
                </div>
                <div className={`p-2 sm:p-3 ${isLight ? 'bg-gray-50 border-gray-200' : 'bg-white/[0.03] border-white/5'} rounded-lg border`}>
                  <div className={`text-[8px] sm:text-[10px] ${isLight ? 'text-gray-500' : 'text-zinc-500'} uppercase tracking-wider`}>Площадок</div>
                  <div className={`text-lg sm:text-xl font-bold ${isLight ? 'text-gray-900' : 'text-white'} mt-0.5 sm:mt-1`}>{selectedPlatforms}</div>
                </div>
                <div className={`p-2 sm:p-3 ${isLight ? 'bg-gray-50 border-gray-200' : 'bg-white/[0.03] border-white/5'} rounded-lg border`}>
                  <div className={`text-[8px] sm:text-[10px] ${isLight ? 'text-gray-500' : 'text-zinc-500'} uppercase tracking-wider`}>Стран</div>
                  <div className={`text-lg sm:text-xl font-bold ${isLight ? 'text-gray-900' : 'text-white'} mt-0.5 sm:mt-1`}>{countries.length}</div>
                </div>
              </div>
            </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          <div className="hidden sm:flex items-center gap-1.5 flex-wrap">
                            ...
                            {track.producers && track.producers.filter(p => p?.trim()).length > 0 && (
                              <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 ${isLight ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : 'bg-gradient-to-r from-indigo-500/15 to-blue-500/15 text-blue-300 border-blue-500/20'} text-[10px] rounded-full border`}>
                            ...
                            {track.featuring && track.featuring.filter(f => f?.trim()).length > 0 && (
                              <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 ${isLight ? 'bg-pink-50 text-pink-700 border-pink-200' : 'bg-gradient-to-r from-pink-500/15 to-rose-500/15 text-pink-300 border-pink-500/20'} text-[10px] rounded-full border`}>
                            ...
                            {!track.isInstrumental && track.hasDrugs && (
                              <span className={`inline-flex items-center justify-center w-[18px] h-[18px] ${isLight ? 'bg-red-100 text-red-700 border-red-300' : 'bg-red-500/15 text-red-400 border-red-500/40'} text-[10px] font-bold rounded border leading-none`}>
                                E
                              </span>
                            )}
                          </div>
                        ...
                        <div className="hidden sm:flex items-center gap-1.5 flex-wrap mt-1">
                          ...
                          {(track.audioFile?.name || track.originalFileName || track.link) && (
                            <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 ${isLight ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-gradient-to-r from-emerald-500/10 to-teal-500/10 text-emerald-400 border-emerald-500/20'} text-[10px] rounded-md border`}>
                          ...
                          {!track.isInstrumental && track.language && (
                            <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 ${isLight ? 'bg-cyan-50 text-cyan-700 border-cyan-200' : 'bg-gradient-to-r from-cyan-500/10 to-sky-500/10 text-cyan-300 border-cyan-500/20'} text-[10px] rounded-md border`}>
                              <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                                <circle cx="12" cy="12" r="10"/>
                          ...
                          {/* Clean */}
                          {!track.isInstrumental && !track.hasDrugs && (
                            <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 ${isLight ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-gradient-to-r from-emerald-500/10 to-green-500/10 text-emerald-400 border-emerald-500/20'} text-[10px] rounded-md border`}>
                          ...
                          {/* Instrumental */}
                          {track.isInstrumental && (
                            <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 ${isLight ? 'bg-violet-50 text-violet-700 border-violet-200' : 'bg-gradient-to-r from-violet-500/15 to-purple-500/15 text-violet-300 border-violet-500/25'} text-[10px] rounded-md font-bold border`}>
                              <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                                <path d="M9 18V5l12-2v13"/>
                                <circle cx="6" cy="18" r="3"/>
                                <circle cx="18" cy="16" r="3"/>
                          ...
                          {/* Версия */}
                          {track.version && (
                            <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 ${isLight ? 'bg-orange-50 text-orange-700 border-orange-200' : 'bg-gradient-to-r from-orange-500/10 to-amber-500/10 text-orange-300 border-orange-500/20'} text-[10px] rounded-md border`}>
                              {track.version}
                            </span>
                          )}
                          {/* Авторы */}
                          {track.authors && track.authors.length > 0 && (track.authors as TrackAuthor[]).map((author, idx) => {
                            ...
                            return (
                              <span key={idx} className={`inline-flex items-center gap-1 px-1.5 py-0.5 ${isLight ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-amber-500/10 text-amber-200 border-amber-500/20'} text-[10px] rounded-md border`}>
                                ...
                                <span className={`${isLight ? 'text-amber-600' : 'text-amber-400'} font-medium`}>{roleLabel}:</span>
                ...
                <div className={`space-y-2 text-sm`}>
                  ...
                  {albumDescription && (
                    <div className={isLight ? 'text-gray-600' : 'text-zinc-400'}>
                      ...
                      <p className={`mt-1 text-xs ${isLight ? 'text-gray-600' : 'text-zinc-400'} line-clamp-2`}>{albumDescription}</p>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className={`mt-6 sm:mt-8 pt-4 sm:pt-6 ${isLight ? 'border-gray-200' : 'border-white/10'} flex flex-col sm:flex-row gap-3 sm:justify-between`}>
          ...
          <button 
            ...
            className={`px-6 sm:px-8 py-3 sm:py-4 rounded-xl font-black transition flex items-center justify-center gap-2 relative overflow-hidden text-sm sm:text-base order-1 sm:order-2 touch-manipulation ${
              allValid && !submitting
          ...
          >
            {submitting ? (
              ...
              <>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-4 h-4 sm:w-5 sm:h-5">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className={`w-10 h-10 rounded-xl ${isLight ? 'bg-emerald-50 hover:bg-emerald-100 border-emerald-200 hover:border-emerald-300' : 'bg-emerald-500/10 hover:bg-emerald-500/20 border-emerald-500/30 hover:border-emerald-500/50'} border flex items-center justify-center transition group`}
                  ...
                  >
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={`${isLight ? 'text-gray-500 group-hover:text-gray-700' : 'text-zinc-400 group-hover:text-zinc-300'}`}>
        ...
        {mounted && showConfirmDialog && createPortal(
          <div 
            className={`fixed inset-0 z-[99999] flex items-center justify-center ${isLight ? 'bg-black/50' : 'bg-black/70'} backdrop-blur-sm`}
            style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, width: '100vw', height: '100vh' }}
          >
            <div className={`${isLight ? 'bg-white border-gray-200' : 'bg-zinc-900 border-white/10'} border rounded-2xl p-6 max-w-md w-full mx-4 shadow-2xl animate-fade-up`}>
              ...
              <div className="flex gap-3">
                ...
                // Создание релиза в базе (Exclusive - бесплатные релизы для exclusive пользователей)
                ...
                if (draftId) {
                  ...
                  if (contributors.length > 0) {
                    ...
                    // Игнорируем ошибки контрибьюторов
                    void contribError;
                  }
                } else {
                ...
                try {
                  ...
                  // Игнорируем ошибку удаления черновиков
                }
                ...
                setSubmitting(false);
                  ...
                  className="flex-1 px-4 py-3 bg-emerald-500 hover:bg-emerald-400 text-white rounded-xl font-bold transition"
                >
                  Да, отправить
                </button>
              </div>
