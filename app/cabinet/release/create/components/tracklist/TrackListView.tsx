  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      ...
      <div className={`mt-6 sm:mt-8 pt-4 sm:pt-6 border-t ${isLight ? 'border-gray-200' : 'border-white/10'} flex justify-between`}>
        <button onClick={onBack} className={`px-4 sm:px-6 py-2.5 sm:py-3 ${isLight ? 'bg-gray-100 hover:bg-gray-200 text-gray-700' : 'bg-white/5 hover:bg-white/10 text-white'} rounded-xl font-bold transition flex items-center gap-2 text-sm sm:text-base touch-manipulation`}>
...
function TrackRow({
  ...
  return (
    <div 
      ...
      className={`relative flex items-center gap-2 sm:gap-4 p-2 sm:p-5 rounded-lg sm:rounded-2xl border-2 transition-all duration-300 ${
        isDragged 
          ...
          : isDragOver 
            ? isLight
              ? 'bg-gradient-to-br from-purple-100 to-blue-100 border-purple-400 shadow-2xl shadow-purple-500/20 scale-[1.02]'
              : 'bg-gradient-to-br from-purple-500/20 to-blue-500/20 border-purple-500 shadow-2xl shadow-purple-500/20 scale-[1.02]'
            : isLight
              ? 'bg-white border-gray-200 hover:border-gray-300 hover:shadow-xl hover:scale-[1.01]'
    ...
    >
      <div className={`absolute inset-0 ${isLight ? 'bg-gradient-to-br from-purple-100/50 via-transparent to-blue-100/50' : 'bg-gradient-to-br from-purple-500/5 via-transparent to-blue-500/5'} rounded-xl sm:rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity`} />
      ...
      <div className="relative flex-shrink-0">
        {coverPreviewUrl ? (
          <div className={`relative w-8 h-8 sm:w-12 sm:h-12 rounded-md sm:rounded-xl overflow-hidden ring-1 ${isLight ? 'ring-gray-200 group-hover:ring-purple-400' : 'ring-white/10 group-hover:ring-purple-500/50'} transition-all`}>
      ...
      <div className="flex-1 relative min-w-0">
        ...
        <div className="flex flex-wrap items-center gap-x-1.5 sm:gap-x-2 gap-y-0.5 sm:gap-y-1 mb-1 sm:mb-1.5">
          ...
          {track.producers && track.producers.length > 0 && track.producers.some((p: string) => p.trim()) && (
            <span className={`inline-flex items-center gap-0.5 px-1.5 sm:px-2 py-0.5 ${isLight ? 'bg-gradient-to-r from-indigo-100 to-blue-100 text-blue-700 border-blue-300' : 'bg-gradient-to-r from-indigo-500/20 to-blue-500/20 text-blue-300 border-blue-500/30'} text-[10px] sm:text-xs rounded-full border`}>
          ...
          {track.featuring && track.featuring.length > 0 && track.featuring.some((f: string) => f.trim()) && (
            <span className={`inline-flex items-center gap-0.5 px-1.5 sm:px-2 py-0.5 ${isLight ? 'bg-gradient-to-r from-pink-100 to-rose-100 text-pink-700 border-pink-300' : 'bg-gradient-to-r from-pink-500/20 to-rose-500/20 text-pink-300 border-pink-500/30'} text-[10px] sm:text-xs rounded-full border`}>
        ...
        <div className="flex flex-wrap items-center gap-x-1.5 sm:gap-x-2 gap-y-0.5 sm:gap-y-1 text-[10px] sm:text-xs">
          {/* Аудиофайл */}
          {(track.audioFile || track.link || track.originalFileName) && (
            <span className={`inline-flex items-center gap-0.5 sm:gap-1 px-1.5 sm:px-2 py-0.5 sm:py-1 ${isLight ? 'bg-gradient-to-r from-emerald-100 to-teal-100 text-emerald-700 border-emerald-300' : 'bg-gradient-to-r from-emerald-500/15 to-teal-500/15 text-emerald-400 border-emerald-500/25'} rounded-md sm:rounded-lg border`}>
              <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-2.5 h-2.5 sm:w-[10px] sm:h-[10px]">
              ...
              {(() => {
                ...
                return (
                  <>
                    ...
                    <span className="hidden sm:inline text-[10px] opacity-70 truncate max-w-[120px]">{nameWithoutExt}</span>
                  </>
                );
              })()}
            </span>
          ...
          {!track.isInstrumental && track.language && (
            <span className={`inline-flex items-center px-1.5 sm:px-2 py-0.5 sm:py-1 ${isLight ? 'bg-gradient-to-r from-cyan-100 to-sky-100 text-cyan-700 border-cyan-300' : 'bg-gradient-to-r from-cyan-500/15 to-sky-500/15 text-cyan-300 border-cyan-500/25'} rounded-md sm:rounded-lg border`}>
              {track.language}
            </span>
          )}
          {/* Clean */}
          {!track.isInstrumental && !track.hasDrugs && (
            <span className={`inline-flex items-center gap-0.5 px-1.5 sm:px-2 py-0.5 sm:py-1 ${isLight ? 'bg-gradient-to-r from-emerald-100 to-green-100 text-emerald-700 border-emerald-300' : 'bg-gradient-to-r from-emerald-500/10 to-green-500/10 text-emerald-400 border-emerald-500/20'} rounded-md sm:rounded-lg font-medium border`}>
              <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="w-2.5 h-2.5 sm:w-[10px] sm:h-[10px]">
          ...
          {/* Версия */}
          {track.version && (
            <span className={`inline-flex items-center px-1.5 sm:px-2 py-0.5 sm:py-1 ${isLight ? 'bg-gradient-to-r from-orange-100 to-amber-100 text-orange-700 border-orange-300' : 'bg-gradient-to-r from-orange-500/15 to-amber-500/15 text-orange-300 border-orange-500/25'} rounded-md sm:rounded-lg border`}>
              {track.version}
            </span>
          )}
          {/* Instrumental */}
          {track.isInstrumental && (
            <span className={`inline-flex items-center gap-0.5 px-1.5 sm:px-2 py-0.5 sm:py-1 ${isLight ? 'bg-gradient-to-r from-violet-100 to-purple-100 text-violet-700 border-violet-300' : 'bg-gradient-to-r from-violet-500/20 to-purple-500/20 text-violet-300 border-violet-500/30'} rounded-md sm:rounded-lg font-bold border`}>
              <span>INSTRUMENTAL</span>
            </span>
          )}
          {/* Авторы */}
          {track.authors && track.authors.length > 0 && (
            <span className={`inline-flex items-center gap-0.5 sm:gap-1 px-1.5 sm:px-2 py-0.5 sm:py-1 ${isLight ? 'bg-gradient-to-r from-amber-100 to-yellow-100 text-amber-700 border-amber-300' : 'bg-gradient-to-r from-amber-500/15 to-yellow-500/15 text-amber-300 border-amber-500/25'} rounded-md sm:rounded-lg border`}>
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className={`w-2.5 h-2.5 sm:w-[10px] sm:h-[10px] ${isLight ? 'text-amber-600' : 'text-amber-400'}`}>
                <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/>
                <circle cx="12" cy="7" r="4"/>
              </svg>
              <span className="truncate max-w-[80px] sm:max-w-none">{track.authors.map((a: any) => a.fullName || a).filter(Boolean).join(', ')}</span>
            </span>
          )}
        </div>
      </div>
...
function TrackStatusMessage({ releaseType, tracksCount, maxTracks, isLight }: TrackStatusMessageProps) {
  if (releaseType === 'single' && tracksCount === 1) {
    return (
      <div className={`flex items-start gap-2.5 sm:gap-3 p-3 sm:p-4 ${isLight ? 'bg-emerald-50 border-emerald-200' : 'bg-emerald-500/10 border-emerald-500/20'} border rounded-xl`}>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" className={`${isLight ? 'text-emerald-600' : 'text-emerald-400'} flex-shrink-0 mt-0.5 w-4 h-4 sm:w-5 sm:h-5`} strokeWidth="2">
          <circle cx="12" cy="12" r="10"/><path d="M12 16v-4M12 8h.01"/>
        </svg>
        <div className="text-xs sm:text-sm">
          <div className={`font-semibold ${isLight ? 'text-emerald-700' : 'text-emerald-400'} mb-0.5 sm:mb-1`}>✓ Сингл готов</div>
  ...
  if (releaseType === 'ep' && tracksCount < 2) {
    return (
      <div className={`flex items-start gap-2.5 sm:gap-3 p-3 sm:p-4 ${isLight ? 'bg-orange-50 border-orange-200' : 'bg-orange-500/10 border-orange-500/20'} border rounded-xl`}>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" className={`${isLight ? 'text-orange-600' : 'text-orange-400'} flex-shrink-0 mt-0.5 w-4 h-4 sm:w-5 sm:h-5`} strokeWidth="2">
          <circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01"/>
        </svg>
        <div className="text-xs sm:text-sm">
          <div className={`font-semibold ${isLight ? 'text-orange-700' : 'text-orange-400'} mb-0.5 sm:mb-1`}>⚠️ Требования для EP</div>
  ...
  if (releaseType === 'album' && tracksCount < 8) {
    return (
      <div className={`flex items-start gap-2.5 sm:gap-3 p-3 sm:p-4 ${isLight ? 'bg-orange-50 border-orange-200' : 'bg-orange-500/10 border-orange-500/20'} border rounded-xl`}>
        ...
        <div className="text-xs sm:text-sm">
          <div className={`font-semibold ${isLight ? 'text-orange-700' : 'text-orange-400'} mb-0.5 sm:mb-1`}>⚠️ Требования для Альбома</div>
